...
- **Vehicle time and late data.** Sessions are built on the vehicle's timestamp. Events arriving behind a watermark trigger re-derivation of a sliding window around them. Alerts fire from one normalized state stream, not from both `raw_states` and sessions, and are de-duplicated through `rule_firings` so a replay cannot re-send "drive started".
