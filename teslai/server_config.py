...
the private-CA server certificate and publishes every record type to Mosquitto:
    ...
    reliable_ack on for V only: the car's data is acked after the MQTT publish succeeds
...
def fleet_telemetry_config(port: int = 4443, mqtt_broker: str = "mosquitto:1883",
    ...
    return {
        ...
        "namespace": "teslai",
        "reliable_ack": True,
        # fleet-telemetry only supports reliable acks for vehicle data (V) records.
